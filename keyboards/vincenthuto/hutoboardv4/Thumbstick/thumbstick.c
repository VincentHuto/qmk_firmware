...
void                   thumbstick_process(void) {
    if (timer_elapsed(thumbstickTimer) > THUMBSTICK_TIMEOUT) {
        ...
        thumbstick_state.vector.x = thumbstick_get_component(ANALOG_JOYSTICK_X_AXIS_PIN);
#else
        thumbstick_state.vector.x = -thumbstick_get_component(ANALOG_JOYSTICK_X_AXIS_PIN);
#endif
#ifndef THUMBSTICK_FLIP_Y
        thumbstick_state.vector.y = thumbstick_get_component(ANALOG_JOYSTICK_Y_AXIS_PIN);
#else
        thumbstick_state.vector.y = -thumbstick_get_component(ANALOG_JOYSTICK_Y_AXIS_PIN);
#endif
